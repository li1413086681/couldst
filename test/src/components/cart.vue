<template>
	<div class="container" style="width: 100%;">
		<top></top>
		<search></search>
		<div class="container" style="width:100%;height: 700px;background-image: url(../../static/imgs/gouwuche.jpg);background-size: 100% 100%;">
...
<script>
	...
	export default{
		...
		methods:{
			...
			deletecart(ctid,i){
				var ob=this;
				var url="http://127.0.0.1:8087/mgj/mgj/deletecart";
				$.ajax(url,{
			...
			countup(ctid,count,i){
				var ob=this;
				var url="http://127.0.0.1:8087/mgj/mgj/changecartcount";
				$.ajax(url,{
					xhrFields: {"withCredentials": true},
					async:false,
					data:{
		...
		mounted(){
			var ob=this;
			var url="http://127.0.0.1:8087/mgj/mgj/getcart";
			$.ajax(url,{
				xhrFields: {"withCredentials": true},
				dataType:"json",
				success: function(result) {
					ob.list=result;
					for(var j in result){
						ob.gdimg.push({"background-image":"url(http://127.0.0.1:8087/mgj/tp/"+result[j].gimgurl+")"});
					}
				}
			});
		}
